  ...
  return <section className={`trend-dock ${expanded ? "expanded" : ""}`}>
    ...
    <div className="simulation-timeline">
      ...
      <div className="timeline-track">
        ...
        {events.slice(0, 14).map((event) => <button aria-label={`${event.title}，T+${event.simulationTime?.toFixed(0) ?? "—"} 秒`} className={`timeline-marker ${event.type}`} key={event.id} onClick={() => event.simulationTime !== null && sourceMode === "replay" && onSeek(event.simulationTime)} style={{left: `${eventPosition(event, timelineDuration)}%`}} title={`${event.title} · ${event.detail}`} />)}
