  ...
  useEffect(() => {
    ...
    void import("./echartsRuntime").then(({initTrafficChart}) => {
      ...
      chart.setOption({
        ...
        legend: {top: 0, right: 0, textStyle: {color: "#789694", fontSize: 9}},
        tooltip: {
          trigger: "axis",
          backgroundColor: "#08191c",
          borderColor: "#31504f",
          textStyle: {color: "#d8e7e5", fontSize: 9},
          valueFormatter: (item: unknown) => typeof item === "number" ? `${item.toFixed(1)}%` : String(item),
        },
        xAxis: {
          type: "category",
          data: chartData.map((item) => item.label),
          axisLine: {lineStyle: {color: "#28413f"}},
          axisLabel: {color: "#67817f", fontSize: 8},
        },
        yAxis: {
          type: "value",
          name: "Baseline = 100",
          nameTextStyle: {color: "#58716f", fontSize: 8},
          axisLabel: {color: "#67817f", fontSize: 8},
          splitLine: {lineStyle: {color: "rgba(96, 132, 129, .1)"}},
        },
        series: [
          {name: baselineLabel || "Baseline", type: "bar", data: chartData.map(() => 100), barMaxWidth: 16, itemStyle: {color: "#345753"}},
          {name: candidateLabel || "Candidate", type: "bar", data: chartData.map((item) => item.candidateIndex), barMaxWidth: 16, itemStyle: {color: "#38d8b5"}},
        ],
      });
